...
{
    public partial class VENTAS_MENU_PRINCIPALOK : DevComponents.DotNetBar.Metro.MetroForm
    {
        ...
        public static double ivgpasarventa;
        string SerialPC;
        string sevendePor;
       ...
       private void sumar()
        {
            try
            {
                ...
                {
                    //PARA SUMAR LOS PRECIOS UNITARIOS DE CADA PRODCUTO
                    double calcularsumasubtotal=0;
                    double calcularsumaIGV = 0;

                    calcularsumasubtotal += Convert.ToDouble  (fila.Cells["Importe"].Value);
                    calcularsumaIGV += Convert.ToDouble(Convert.ToDouble(fila.Cells["Impuesto"].Value)* calcularsumasubtotal);


                    totalpagar += calcularsumasubtotal;
                    impuestoIGV += calcularsumaIGV;

                    //lbligv.Text= Convert.ToString((totalpagar*0.18)+ totalpagar);
                    lbligv.Text = impuestoIGV.ToString();
                   

                    txt_total_suma.Text =Convert.ToString ( totalpagar + impuestoIGV);
                    lbltotalfinal.Text = Convert.ToString(totalpagar + impuestoIGV);

                    lblsubtotal.Text = totalpagar.ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void vender_por_teclado()
        {
            ...
            Impuestoproducto = DATALISTADO_PRODUCTOS_OKA.SelectedCells[11].Value.ToString();
       ...
       private void mostrar_panel_de_Cobro()
        ...
        private void insertar_detalle_venta_Validado()
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Impuesto", Impuestoproducto);
                cmd.ExecuteNonQuery();
                con.Close();
  ...
  if (txtbuscar.Text =="")
        ...
        {
            //pasamos los datos al siguiente formulario.
            total = Convert.ToDouble(txt_total_suma.Text);
            ivgpasarventa = Convert.ToDouble(lbligv.Text);
